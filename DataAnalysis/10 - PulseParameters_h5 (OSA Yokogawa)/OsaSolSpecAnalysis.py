...
def plotPeakDataWithFit(resultDict):
    ...
    # plot fit
    plt.plot(peakFreqs_THz, pulseEnvelopeDBM(peakFreqs_THz, *fitValues), 'g--',
             label=labelStr)
    # plot background
...
def plotDataWithFit(dataDict, resultDict):
    ...
    fig, ax = osa.plotTraceFromDataDict(dataDict, xScaleFreqs=True)
...
def main():
    ...
    # plotting
    plotPeakDataWithFit(fittingResultDict)
