...
folder                  = '_Quality_factor_1550nm_chips/'
...
import os
directory_data = save_path + project + main_folder+ date + folder + \
...
begin_scan  = float(files[0][len(file):-3])
finish_scan = float(files[-1][len(file):-3])

step_size   = np.round(float(files[1][len(file):-3])-float(files[0][len(file):-3]),2)
###############################################################################
...
###     We reduce the window only on the resonance which we are interested in                
        ...
        #                plt.plot(freq[peaks],voltageTrace[peaks],'go')
                    ...
                    Linewidth.append(FHWM)

                    
                
                
print('Traces taken: %s ; Traces used: %s' % (number_of_traces,count_))
...
if plot_hist == True:
    plt.figure(figsize=size_fig, dpi=fig_resolution)
    plt.title(str(measurementPurposeShort[:-1]))
...
if plot_hist == True:    
    intervals = 100
    plt.figure(figsize=size_fig, dpi=fig_resolution) 
    plt.hist(Q_plot,bins = intervals, alpha=0.5)
    plt.title(str(measurementPurposeShort[:-1]))    
    plt.xlabel('Quality factor (mill.)' ,fontsize=20)
...
if plot_hist == True:    
    intervals = 1000
    plt.figure(figsize=size_fig, dpi=fig_resolution) 
    plt.hist(Linewidth,bins = intervals, alpha=0.5)
    plt.title(str(measurementPurposeShort[:-1]))    
