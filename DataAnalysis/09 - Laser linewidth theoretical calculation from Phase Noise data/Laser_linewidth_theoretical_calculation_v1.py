...
Following the paper: 
Domenico et al., "Simple approach to the relation between laser frequency noise 
...
from Plot_aux_functions import Plot, Plot_parameters, makeTable # Import all functions from the script
...
def getValuesfromTXT(fileName, headers=['Frequency', 'PSD']):
    """Reads phase noise data from a txt file and returns a pandas dataframe.
    and returns PSD Phase noise vs Fourier Frequency"""
...
def getValuesfromCSV(fileName, sep=',', headers=['Frequency', 'PSD']):
    """Reads phase noise data from a csv file and returns a pandas dataframe 
    and returns PSD Phase noise vs Fourier Frequency """
...
def ReadFile(fileName):
    if 'csv' in fileName:
        ...
        frequency, psd =  getValuesfromTXT(fileName)
    else:
...
if __name__ == '__main__':
    ...
    folder = r'\\menloserver\MFS\03-Operations\02-DCP\01-DCP_Management\01-Gruppenmanagement\UMS\05_LabTools_TBD\09-Laser linewidth theoretical calculation from Phase Noise data\\'
    fileName = '20210419-142704_Data_PSD.txt'

    frequency, psd = ReadFile(folder+fileName)        
    ...
    fig = Plot(frequency,psd,log_scale=True, yLabel='ℒ(f) [dBc/Hz]',num=2, 
          label = f'Phase Noise data\nLinewidth = {FWHM/factor:.4} {FWHM_unit}Hz')
    
    print('FWHM value obtained experimentally is < 1 Hz !!!')
    
    freq_table = [10,100,1000,10000]
    frequency = np.array(frequency)
    psd = np.array(psd)
    makeTable(frequency, psd, fig, print_freqs=freq_table, x_pos = 0.5, y_pos=0.5)
