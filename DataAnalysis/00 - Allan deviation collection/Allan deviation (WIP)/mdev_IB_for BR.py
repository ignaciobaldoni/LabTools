...
# columnno = 7
# #load Data
# a1 = np.genfromtxt(dataFolderpath + '230331_1_Frequ.txt', dtype=float, comments='#', delimiter=None, converters=None, usecols=(columnno), unpack=False,invalid_raise=False)
# a2 = np.genfromtxt(dataFolderpath + '230401_1_Frequ.txt', dtype=float, comments='#', delimiter=None, converters=None, usecols=(columnno), unpack=False,invalid_raise=False)
# a3 = np.genfromtxt(dataFolderpath + '230402_1_Frequ.txt', dtype=float, comments='#', delimiter=None, converters=None, usecols=(columnno), unpack=False,invalid_raise=False)
# a4 = np.genfromtxt(dataFolderpath + '230403_1_Frequ.txt', dtype=float, comments='#', delimiter=None, converters=None, usecols=(columnno), unpack=False,invalid_raise=False)
# an = np.append(a1,a2)
# ann = np.append(an,a3)
...
# Dataf = c.to_numpy()
# Data_selected = Dataf[int(anfang):int(ende), :]

# Data = pd.DataFrame(Data_selected, columns=['Time', 'Frequency'])

# # import matplotlib.pyplot as plt

# # plt.plot(Data.Time,Data.Frequency)

# Data.to_csv('filename_BR.csv', index=False)

filenames = ['filename_BR.csv']
...
for i in filenames:
    ...
    dataName = f'BR forma boluda de hacer cuentas_{i}'
    wavelength = 1542e-9
    
    interval_sec = 0.1
    
    createPlot(resultsFolderpath,dataFilePath,dataName,interval_sec,wavelength,column_number = 1)
