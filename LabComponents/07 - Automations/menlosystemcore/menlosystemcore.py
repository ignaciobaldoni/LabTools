...
class MenloSystemCore:
    ...
    def systemLogic(self):
        """ The system logic exported by the MenloSystemCore instance

        This was removed from menlosystemcore in version 1.3.0.
        """
        if not self._webChannel:
            return
        if not hasattr(self._webChannel.objects["root"], 'systemLogic'):
            return None
        return self._webChannel.objects["root"].systemLogic


    @property
    def automationLayer(self):
        """ The automation layer exported by the MenloSystemCore instance

        This was added to menlosystemcore in version 1.3.0.
        """
        if not self._webChannel:
            return
        if not hasattr(self._webChannel.objects["root"], 'automationLayer'):
            return None
        return self._webChannel.objects["root"].automationLayer
    ...
    @property
    def functionalLayer(self):
        """ All functional layer exported by the MenloSystemCore instance

        This was added to menlosystemcore in version 1.3.0.
        """
        if not self._webChannel:
            return
        if not hasattr(self._webChannel.objects["root"], 'functionalLayer'):
            return None
        return self._webChannel.objects["root"].functionalLayer
    ...
    def enumerators(self):
        """ All enumerators used by objects exported by the MenloSystemCore
        instance
        """
        if not self._webChannel:
            return
        ...
        enumMap = {}
        for k, v in self._webChannel.objects["root"].enumerators.items():
            enumMap[k] = enum.Enum(k, v)
    ...
    async def connect(self, url=None, user=None, password=None, loop=None):
        ...
        transport = await ws_connect(url, **ssl_opts, loop=loop,
                                     max_size=20 * 1024 * 1024 # 20 MiB max size
                                     )
