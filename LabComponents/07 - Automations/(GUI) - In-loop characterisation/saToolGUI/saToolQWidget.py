# -*- coding: utf-8 -*-
"""
Created on Mon Nov  8 10:01:21 2021
...
class saToolQWidget(QGroupBox):
    
    # %%% init
    def __init__(self, parent=None, fileMetaData = None,**kwds):
        super(saToolQWidget, self).__init__(parent)
        ...
        readAndSaveSpectrumTracePushButton = QPushButton("Read and Save Trace Data")
        readAndSaveSpectrumTracePushButton.setDefault(True)
        readAndSaveSpectrumTracePushButton.clicked.connect(self.readAndSaveSpectrumTrace)
        
        plotSpectrumTracePushButton = QPushButton("plot Trace")
        plotSpectrumTracePushButton.setDefault(True)
        plotSpectrumTracePushButton.clicked.connect(self.plotSpectrumTrace)
        ...
        layout.addWidget(readAndSaveSpectrumTracePushButton)
        layout.addWidget(plotSpectrumTracePushButton)
    ...
    def setRadioButtonGroup(self):
        ...
        self.cs_group.addButton(b5, 5)
        self.cs_group.addButton(b6, 6)
    ...
    def smartSave(self):
        ...
        self.readAndSaveSpectrumTrace(dataFilePath = filePath,commentText = commentText)
        ...
        self.plotSpectrumTrace(commentText = commentText)
    ...
    def readAndSaveSpectrumTrace(self, * , dataFilePath = None,commentText = None):
        if self.vxaTool != None:
            ...
            self.traceDataDict = self.vxaTool.readSpectrumTraceData(traceNumber)
    ...
    def plotSpectrumTrace(self , * ,commentText = None):
        if self.vxaTool != None:
            traceNumber = self.cs_group.checkedId()
            print(traceNumber)
            self.vxaTool.plotSpectrumTraceDataComment(traceNumber,commentText)          
