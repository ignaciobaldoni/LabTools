...
class pNoiseToolQWidget(QGroupBox):
    
    # %%% init
    def __init__(self, parent=None, fileMetaData = None,**kwds):
        super(pNoiseToolQWidget, self).__init__(parent)
        ...
        readAndSavePNoiseTracePushButton = QPushButton("Read and Save Trace Data")
        readAndSavePNoiseTracePushButton.setDefault(True)
        readAndSavePNoiseTracePushButton.clicked.connect(self.readAndSavePNoiseTrace)
        
        plotPNoiseTracePushButton = QPushButton("plot Trace")
        plotPNoiseTracePushButton.setDefault(True)
        plotPNoiseTracePushButton.clicked.connect(self.plotPNoiseTrace)
        ...
        layout.addWidget(readAndSavePNoiseTracePushButton)
        layout.addWidget(plotPNoiseTracePushButton)
    ...
    def smartSave(self):
        ...
        self.readAndSavePNoiseTrace(dataFilePath = filePath,commentText = commentText)
        ...
        self.plotPNoiseTrace(commentText = commentText)
    ...
    def connectToPXA(self):                                                                                     
        ...
        # self.vxaTool = AgilentN9030A(resourceStr)
    ...
    def readAndSavePNoiseTrace(self, * , dataFilePath = None,commentText = None):
        if self.vxaTool != None:
            ...
            self.traceDataDict = self.vxaTool.readPhaseNoiseTrace([traceNumber])
    ...
    def plotPNoiseTrace(self , * ,commentText = None):
        if self.vxaTool != None:
            traceNumber = self.cs_group.checkedId()
            print(traceNumber)
            self.vxaTool.plotPhaseNoiseTrace(traceNumber,commentText)          
