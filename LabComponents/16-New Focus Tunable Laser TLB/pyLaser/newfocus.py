...
class NewFocus6700(object):
    ...
    def Track_Off(self):
        word = 'OUTP:TRACK?'
        self._trackoff = self.Query(word)
        return self._trackoff
    
    @Track_Off.setter
    @InOut.accepts(bool)
    def Track_Off(self,value):
        word = 'OUTP:TRACK 0'.format(int(value))
        self.Query(word)
        self._trackoff = value
        
    @property
    @InOut.output(float)
    def power(self):
        word = 'SENSe:POWer:DIODe?'#'OUTPut:STATe?'
        self._power = self.Query(word)
        return self._power
    ...
    def power(self,value):
        self._targetpower = value
        word = "SOUR:POW:DIOD {}".format(value)#"OUTPut:STATe {}".format(int(value))
        self.Query(word)
        self._power = value
